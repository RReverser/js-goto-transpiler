...
var visitor = types.PathVisitor.fromMethodsObject({
  ...
  visitFunction: function(path) {
    ...
    if (!node.generator && !shouldTransformAsync && !path.__hasGotos) {
      return;
    }
    ...
    var wrapCall = b.callExpression(
      runtimeProperty(shouldTransformAsync ? "async" : node.generator ? "wrap" : "sync"),
      wrapArgs
    );
  ...
  visitLabeledStatement: function (path) {
    if (path.get("label", "name").value === "goto") {
      do {
        path = path.parent;
      } while (path && !n.Function.check(path.node));
      if (!path) {
        throw new Error("Gotos outside of functions are not supported.");
      }
      path.__hasGotos = true;
      return false;
    }
    this.traverse(path);
  }
