...
Ep.mark = function(loc, label) {
  ...
  this.marked[index] = label ? label.name : true;
  return loc;
};
...
Ep.getDispatchLoop = function() {
  ...
  self.listing.forEach(function(stmt, i) {
    if (self.marked.hasOwnProperty(i)) {
      if (typeof self.marked[i] === "string") {
        cases.push(b.switchCase(
          b.literal(self.marked[i]),
          []
        ));
      }
...
function isSwitchCaseEnder(stmt) {
  return n.BreakStatement.check(stmt)
      ...
      || n.LabeledStatement.check(stmt) && stmt.label.name === "goto";
}
...
Ep.explodeStatement = function(path, labelId) {
  ...
  case "LabeledStatement":
    ...
    if (stmt.label.name === "goto") {
      n.ExpressionStatement.assert(stmt.body);
      var expr = stmt.body.expression;
      n.Identifier.assert(expr);
      var name = expr.name;
      self.jump(self.leapManager.getContinueLoc(expr) || b.literal(expr.name));
      break;
    }
    ...
    self.mark(before, stmt.label);

    self.leapManager.withEntry(
      new leap.LabeledEntry(after, before, stmt.label),
      function() {
